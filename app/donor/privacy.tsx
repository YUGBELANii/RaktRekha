    ...
    <View style={{ flex: 1 }}>
      <ScrollView style={styles.container}>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.rowBetween}>
            <Text>Show My Profile To Nearby Receiver</Text>
        ...
        <View style={styles.card}>
          ...
          <TouchableOpacity style={styles.credRow} onPress={() => setShowChangePassword(!showChangePassword)}>
            <Text>Change Password</Text>
            <Ionicons name={showChangePassword ? 'chevron-up' : 'chevron-forward'} size={18} />
          </TouchableOpacity>

          <TouchableOpacity style={styles.credRow} onPress={() => Alert.alert('Action Blocked', 'Use web version to change number/email')}>
            <Text>Change Number/Email</Text>
            <Ionicons name="chevron-forward" size={18} />
          </TouchableOpacity>

          <TouchableOpacity style={styles.credRow} onPress={() => Alert.alert('Action Blocked', 'Use web version to delete your account')}>
            ...
            <View style={{ marginTop: 10 }}>
              {user?.providerData[0]?.providerId !== 'password' ? (
                <Text style={{ fontSize: 13, color: 'red' }}>
                  Your account is linked via {user?.providerData[0]?.providerId}. Please use web version to update password.
                </Text>
              ) : (
                <>
                  <TextInput
      ...
      <View style={styles.navBar}>
        <Tab icon="home" label="Home" path="/donor/home" />
         <Tab icon="medkit" label="request" path="/donor/request"  />
     <Tab icon="map" label="Camps" path="/donor/camps" />
      <Tab icon="settings" label="Settings" path="/donor/settings" actives/>
      </View>
    </View>
  );
}
